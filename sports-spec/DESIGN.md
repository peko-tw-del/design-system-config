...
本版色彩白名單來自新版 foundation (`8359:14643`) 與 Figma Variables API / foundation metadata 讀取的 18 個 color variables。文字、間距、圓角與陰影已同步 foundation:spacing 4/6/8/10/12/16/20 + safe-bottom 34;radius 4/8/16/full;shadow/nav 與 shadow/pop-up window(舊名 bottom-bar)。元件規格仍有部分歷史抽樣資料,不得覆蓋 foundation 尺度。
